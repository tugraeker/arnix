...
const {
    ...
    error,
    count,
  } = useSupabaseQuery<ProjectTask>("tasks", {
    select: "*",
...
export default function TasksPage() {
  ...
  const byStatus = (s: TaskStatus) => filtered.filter((t) => (tasks ?? TASKS).status === s);

  const members = Array.from(new Map((tasks ?? TASKS).map((t) => [t.assignee?.name ?? "", t.assignee]).filter(([n]) => n)).values());
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge variant="muted" className="!text-[10px]">
              {((tasks ?? TASKS).length)} görev · {((tasks ?? TASKS).filter(t => t.status === "done").length)} tamamlandı
            </Badge>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {COLUMNS.map((c) => {
          const count = (tasks ?? TASKS).filter(t => t.status === c.key).length;
          return (
            <Card key={c.key} className="p-4">
              <CardContent className="p-0">
                ...
                <div className="text-[11px] text-muted-foreground">
                  {c.key === "in_progress" && `{(tasks ?? TASKS).filter(t => t.status === "in_progress").some(t => t.priority === "urgent") ? "⚠ Acil görev mevcut" : "Akıcı"}`}
                  {c.key === "todo" && `{(tasks ?? TASKS).filter(t => t.status === "todo" && t.priority === "urgent").length} acil bekliyor`}
                  {c.key === "review" && `Ortalama 1.3 gün inceleme`}
                  {c.key === "done" && `Haftada ${Math.round(count * 0.7)} tamamlanıyor`}
                </div>
              </CardContent>
            </Card>
          );
      ...
      <Card className="p-4">
        <CardContent className="p-0 grid md:grid-cols-5 gap-3">
          <div className="md:col-span-2 relative">
            ...
            <Input placeholder="Görev ara — başlık, açıklama, etiket..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
          </div>
          ...
          <div className="flex items-center justify-end gap-1.5 p-1 rounded-lg bg-white/[0.03] border border-white/5">
            <Button variant="ghost" size="iconSm" className="text-muted-foreground hover:text-foreground" title="Liste">
      ...
      <Tabs defaultValue="kanban" className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            {COLUMNS.map((col) => (
              <div key={col.key} className="min-w-0">
                ...
                <div className={cn(
                  "rounded-2xl border border-white/5 bg-white/[0.015] p-2 space-y-2 min-h-[300px]",
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/5 bg-white/[0.02]">
                    ...
                    <th className="text-left p-3.5 font-medium text-xs uppercase tracking-wider text-muted-foreground hidden md:table-cell">Proje / Beat</th>
                ...
                <tbody>
                  {filtered.map((t) => {
                    ...
                    const taskSubtasksDone = t.subtasksDone ?? 0;
                    const taskSubtasksTotal = t.subtasksTotal ?? 0;
                    const taskComments = t.comments ?? 0;
                    ...
                    return (
                      <tr key={t.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                        <td className="p-3.5">
                          <div className="flex items-start gap-2">
                            ...
                            <div className="min-w-0">
                              <div className={cn("font-medium text-sm leading-snug", taskStatus === "done" && "line-through text-muted-foreground")}>
                                {t.title}
                              </div>
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                {(taskSubtasksTotal ?? 0) > 0 && (
                                  <span className="text-[10px] text-muted-foreground inline-flex items-center gap-1">
                                    <CheckSquare className="w-2.5 h-2.5" />
                                    {taskSubtasksDone}/{taskSubtasksTotal}
                                  </span>
                                )}
                                {(taskComments ?? 0) > 0 && (
                                  <span className="text-[10px] text-muted-foreground">💬 {taskComments}</span>
                                )}
                                {taskTags.slice(0, 3).map((tag) => (
                                  <Badge key={tag} variant="muted" className="!text-[9px] !px-1.5">
                                    {tag}
                                  </Badge>
                                ))}
                              </div>
                        ...
                        <td className="p-3.5 hidden md:table-cell">
                          {taskBeat.id ? (
                            <Link href={`/beats/${taskBeat.id}`} className="inline-flex items-center gap-2 hover:text-primary">
                        ...
                        <td className="p-3.5">
                          {t.assignee ? (
                            <div className="flex items-center gap-2">
                              ...
                              <div className="min-w-0">
                                <div className="text-xs font-medium truncate">{t.assignee.name}</div>
                                <Badge variant="muted" className={cn("!text-[8px] !px-1 mt-0.5", ROLE_COLORS[t.assignee.role])}>
                                  {ROLE_LABELS[t.assignee.role]}
                                </Badge>
                              </div>
                            </div>
                          ) : (
                        ...
                        <td className="p-3.5 hidden lg:table-cell">
                          <Badge variant="muted" className={cn("!text-[9px] !px-1.5", PRIORITY_COLORS[taskPriority])}>
                        ...
                        <td className="p-3.5 hidden lg:table-cell">
                          <span className="inline-flex items-center gap-1.5 text-xs">
                            {(() => {
                              ...
                              return <Icon className={cn("w-3.5 h-3.5",
                                taskStatus === "done" && "text-neon-green",
                                taskStatus === "in_progress" && "text-primary animate-pulse",
                                taskStatus === "review" && "text-neon-orange"
                              )} />;
                            })()}
        ...
        <TabsContent value="calendar">
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
            {["Bu Gün", "Bu Hafta", "Gelecek Hafta", "Bu Ay", "Geçmiş", "Tarihsiz"].map((group, gi) => {
              const groupTasks = filtered
                .filter(t => gi === 5 ? !t.dueDate : t.dueDate)
                .filter((t, i) => i % 6 === gi || (gi === 5 && !t.dueDate));
              return (
                <Card key={group}>
                  ...
                  <CardContent className="space-y-2">
                    {groupTasks.map((t) => (
                      <div key={t.id} className="p-3 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04]">
                        <div className="flex items-center justify-between mb-1.5">
                          <Badge variant="muted" className={cn("!text-[9px] !px-1.5", PRIORITY_COLORS[t.priority])}>
                            {TASK_PRIORITY_LABELS[t.priority]}
                          </Badge>
                          {t.dueDate && <span className="text-[10px] text-muted-foreground">{formatDate(t.dueDate)}</span>}
                        </div>
                        <div className={cn("text-sm font-medium leading-snug mb-1.5", t.status === "done" && "line-through text-muted-foreground")}>
                          {t.title}
                        </div>
                    ...
                    {groupTasks.length === 0 && (
                      <div className="p-6 text-center text-xs text-muted-foreground italic">Görev yok</div>
                    )}
                  </CardContent>
                </Card>
              );
...
function TaskCard({ task }: { task: TaskItem }) {
  ...
  const progress = (task.subtasksTotal ?? 0) > 0
    ? Math.round((task.subtasksDone ?? 0) / task.subtasksTotal! * 100)
    : null;

  return (
    <div className="group relative p-3.5 rounded-xl border border-white/5 bg-card hover:border-primary/30 hover:shadow-[0_8px_30px_-10px_hsl(var(--primary)/0.25)] transition-all cursor-pointer animate-slide-up">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
          <button className="p-1 rounded-md hover:bg-white/5 text-muted-foreground hover:text-foreground">
      ...
      <div className={cn("text-sm font-medium leading-snug mb-2", done && "line-through text-muted-foreground")}>
        {task.title}
      </div>
      ...
      {progress !== null && (
        <div className="mb-3">
          <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1">
            <span className="inline-flex items-center gap-1"><CheckSquare className="w-2.5 h-2.5" />Alt-görevler</span>
            <span className="font-mono">{task.subtasksDone}/{task.subtasksTotal}</span>
          </div>
          <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-neon-green to-accent transition-all"
      ...
      {task.beat && (
        <Link href={`/beats/${task.beat.id}`} className="mb-3 flex items-center gap-2 p-2 rounded-lg bg-white/[0.02] border border-white/5 hover:bg-primary/5 hover:border-primary/20 transition-colors">
          <Disc3 className="w-3.5 h-3.5 text-primary shrink-0" />
          <span className="text-[11px] truncate flex-1 min-w-0">{task.beat.title}</span>
          <Badge variant="muted" className="!text-[8px] !px-1">
            {BEAT_TYPE_LABELS[task.beat.type]}
          </Badge>
        </Link>
      )}
          ...
          {task.assignee ? (
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0 hidden sm:block">
                <div className="text-[11px] font-medium truncate">{task.assignee.name}</div>
              </div>
            </div>
          ) : (
            <Badge variant="muted" className="!text-[9px]">Atanmamış</Badge>
        ...
        <div className="flex items-center gap-3 text-[10px] text-muted-foreground shrink-0">
          <span className="inline-flex items-center gap-1">
            <StatusIcon className={cn("w-3 h-3",
              ...
              task.status === "review" && "text-neon-orange"
            )} />
          </span>
          {task.dueDate && (
            <span className="inline-flex items-center gap-1">
